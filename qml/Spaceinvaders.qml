import QtQuick 2.4
import QtQuick.Controls 1.0
import QtQuick.Controls.Styles 1.4
import QtQuick.Layouts 1.1
import Spaceinvaders 1.0
import QtQuick.Window 2.2

GameSceen {
    id: mainForm
    width: 640
    height: 480

    //### FOR USE TO CHECK WHICH ORIENTATION THE USER USES
    property bool isPortrait: width < height

    property alias leftButton: leftButton
    property alias rightButton: rightButton

//    GameBar {
//        id: gameBar
//        firstLife.visible: mainForm.firstLife
//        secLife.visible: mainForm.secLife
//        thirdLife.visible: mainForm.thirdLife
//        score.text: "Score: " + mainForm.score
//    }

    /**
      * For checking which orientation
      */
    function checkOrientation() {
        isPortrait = width < height
        if(isPortrait)
            state = "Portrait";
        else if (!isPortrait)
            state = "Landscape";
        orientation = isPortrait;
    }

    //### WHEN WIDTH OR HEIGHT WAS CHANGED
    onWidthChanged: checkOrientation()
    onHeightChanged: checkOrientation()

    //### WHEN THE APP IS COMPILED AND AT THE START CHECK ORIENTATION
    Component.onCompleted: checkOrientation()

    RowLayout {
        id: bottomRow
        spacing: 5
        visible: Qt.platform.os == "android"
        anchors.horizontalCenter: parent.horizontalCenter
        anchors.bottom: parent.bottom
        anchors.bottomMargin: 0
        MoveButton {
            id: leftButton
            text: "Move Left"
        }
        MoveButton {
            id: rightButton
            text: "Move Right"
        }
    }

    //### STATES FOR THE TWO ORIENTATION
    states: [
        State {
            name: "Portrait"
            PropertyChanges { target: bottomRow; spacing: Screen.pixelDensity * 15 }
        },
        State {
            name: "Landscape"
            PropertyChanges { target: bottomRow; spacing: Screen.pixelDensity * 30 }
        }

    ]
}
