    ...
    :GLItem(parent)
{
    ...
    //### SET UP TESTOBJECTS... [...]
#ifdef Q_OS_ANDROID
    m_skybox = new GLSkyBox("MySkyBox",50.0, GLColorRgba::clWhite);
    m_skybox->setTextureFile(":/starfield");
#else
    m_skybox = new GLSkyBox("MySkyBox",22.0, GLColorRgba::clWhite);
    m_skybox->setTextureFile(":/starfield");
#endif
...
//        m_eye = 80.0 * -QVector3D(0.0,-0.5,1.0);
    ...
    //## for testing shoots evrytime it can
//    if (etes){
...
void GameSceen::keyPressEvent(QKeyEvent *event)
{
    if (!event->isAutoRepeat()){
        ...
        if(event->key() == Qt::Key_Left || event->key() == 0x01000012 || event->key() == 0x41)
            m_leftKeyPressed = true;
        else if (event->key() == Qt::Key_Right || event->key() == 0x01000014 || event->key() == 0x44)
        ...
        /*switch (event->key()) {
        // left and a
        case Qt::Key_Left: case 0x01000012: case 0x41:
            m_leftKeyPressed = true;
            break;
        // right and d
        case Qt::Key_Right: case 0x01000014: case 0x44 :
